...
* Time module is very handy too for pausing code for a few seconds so that the user doesn't get lost when a message prompts. It's also handy for showing data one at a time. Used mainly in viewing worksheets such as View patients option.

* I added a code to validate if entered patient name contains numbers or if name is empty. If patient's name are only numbers or contain any number or an empty string an error message will prompt for the user to enter the name again.
